...
from customized_website_crawler.classItem import Item
...
SOURCE = '135_csr'
MAX_THREADS  = 12  # 线程数量
BASE_URL = "https://www.csr.ru/ru/news/"   # 新闻列表页



CARD_SELECTOR = 'css:.news_list a '        # 每条新闻卡片
TIME_SELECTOR = 'css:.bottom'  # 必须要在CARD_SELECTOR后面紧接着使用
...
def crawl_today_news_by_drissionpage(baseUrl:str) -> list[Item]:
    ...
    try:
        ...
        for i in range(MAX_SCROLLS):
            ...
            lastNewTime = news_eles[-1].ele(TIME_SELECTOR).text.replace('Читать', '').strip()
            # print(lastNewTime)
            lastNewTime = datetime.strptime(lastNewTime, '%d.%m.%Y')
            ...
            for news in news_eles[:]:
                ...
                try:
                    ...
                    news_title = news.s_ele('css:.text h3').text
                ...
                # time
                time = news.ele(TIME_SELECTOR).text.replace('Читать', '').strip()
            ...
            if (datetime.now(lastNewTime.tzinfo).date() - lastNewTime.date()).days > TIME_THRESHOLD:
                ...
                break
            # 执行翻页
            next_page_ele = home_tab.ele('css:.pg-right')
            next_page_ele.click()
